...
| `modes/discover-jobs.md` | Skill/tool-keyword-based (never title-based) job discovery — run every 7h by an autonomous cloud routine (WebSearch) and available for manual local runs (Nimble Web Search Agent). Writes to the hosted tracker — an Artifact-hosted page (https://claude.ai/artifact/X4QZdiqn9kecJrezmrmyoE), not a Google Sheet: the Drive connector available in this environment has no Sheets values API for row-level appends — see `docs/superpowers/specs/2026-09-15-skill-based-job-discovery-design.md`. |
